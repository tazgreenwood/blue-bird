...
export default function Likes({ tweet, addOptimisticTweet }: { tweet: TweetWithAuthor; addOptimisticTweet: (newTweet: TweetWithAuthor) => void  }) {
    ...
    return <button onClick={handleLikes} className="flex items-center group">
        <svg 
        ...
        <span className={`group-hover:text-red-600 ml-2 text-sm ${ tweet.user_has_liked_tweet ? "text-red-600" : "text-gray-500"}`}>
            {tweet.likes}
        </span>
    </button>
}